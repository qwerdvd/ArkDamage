...
from .CalDpsSeries import cal_, cal_3d
...
char_curve_3d = on_command("3d干员曲线", priority=5, block=True)
...
# 3D干员曲线
# "3D干员曲线 能天使 满潜 精二90 三技能 一模"
@char_curve_3d.handle()
async def send_char_curve_msg(
    ...
    char = Character(char_info)
    enemy = Enemy(default_enemy)
    img = await cal_3d(char_info, char, enemy)
    if img is not None:
